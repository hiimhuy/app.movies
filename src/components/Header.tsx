        ...
        <Container>
          <div className="flex lg:justify-start justify-between items-end">
            ...
            <div className="hidden lg:flex lg:gap-4 lg:px-12 lg:text-base lg:items-end font-semibold ">
             ...
             {path !== '/search' && <form onSubmit={handleSubmit}>
                <div className="flex items-center gap-2">
                  <div className="relative top-0 border rounded-sm font-semibold focus:border-black">
                    ...
                    <div className="absolute border-s-[2px] bg-white inset-y-0 right-0 flex items-center px-3 text-gray-500 cursor-pointer">
                      {searchResult && searchResult?.data?.items.length > 0? <Close onClick={()=> {setSearchResult(null); setKeyword('')}}/> :<Link href={`/search?keyword=${keyword}`}><Search /></Link>}
                    </div>
                  </div>
                  <div
                  ...
                  <Link onClick={()=>setSearchResult(null)} className="flex justify-center pb-2 hover:text-[#f23f51] border-t-[1px]" href={`/search?query=${keyword}`}>Xem thêm</Link>
                  </div>
                </div>
              </form>}
            </div>
